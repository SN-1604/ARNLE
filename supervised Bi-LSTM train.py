...
parser.add_argument('--epoch', type=int, default=10, help='epoch for training the model')
...
label_train = [dic_host[i] for i in args.label_train]
data_val = np.load(args.data_val)
# label_val = open(args.label_val).read().splitlines()
label_val = np.load(args.label_val)
label_val = [dic_host[i] for i in label_val]
...
with tf.Session() as sess:
    ...
    for i in range(args.epoch):
        with tqdm(total=args.data.shape[0]//args.batchsize) as pbar:
            for batch in get_batch(args.data, args.label_train, args.batchsize, sequence_length, True):
